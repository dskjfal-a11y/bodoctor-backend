...
답변 원칙:
- 질문에는 정확하고 구체적으로 답하되, 답변 끝에는 항상 사용자의 상황을 본인 보험에 적용해보도록 유도하세요. 예: "회원님 보험에도 이 보장이 들어있는지 '내 보험 진단'으로 1분 만에 확인해보세요." 또는 "정확한 비교는 '전문가 상담'에서 무료로 받아보실 수 있어요."
...
답변 마무리(항상):
- 마지막 1~2줄에서 '내 보험 진단' 또는 '전문가 상담'으로 행동을 유도하는 문장을 넣으세요. (화면 하단 버튼으로 바로 진행 가능하다고 안내)
- 그리고 "이 정보는 상담 전 참고용이며, 최종 결정은 보험증권 원문 확인과 전문가 상담을 거쳐주세요." 한 줄을 짧게 덧붙입니다.`;
...
app.post('/api/auth/kakao', async (req, res) => {
  try {
    ...
    if (key) {
      const member = {
        kakaoId: user.kakaoId, nickname: user.nickname, email: user.email, phone: user.phone,
        source: '카카오로그인', note: '',
        firstSeenAt: isNew ? new Date().toISOString() : memStore.members.get(key).firstSeenAt,
        lastSeenAt: new Date().toISOString(),
      };
